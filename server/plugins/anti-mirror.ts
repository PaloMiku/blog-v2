...
function handleMirror(sourcesEncoded: string[], targetEncoded: string) {
    ...
    // 增强域名匹配逻辑 ⬇
    const isBadMirror = sources.some(domain => 
        location.hostname.endsWith(domain) || 
        location.hostname.includes(domain.replace(/\./g, '-'))
    )
    if (isBadMirror) {
        // 添加调试日志 ⬇
        console.debug(`[anti-mirror] Redirecting from ${location.host} to ${target}`)
        if (canonical)
